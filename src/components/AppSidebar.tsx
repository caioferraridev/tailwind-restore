...
import {
  LayoutDashboard, Users, Briefcase, ListChecks, Calendar,
  Target, DollarSign, Settings, LogOut, BarChart3, Sparkles, UserCog, Activity
} from "lucide-react";
import {
    ...
    <Sidebar collapsible="icon" className="border-r-0">
      <SidebarHeader className="border-b border-sidebar-border p-4">
        <div className="flex items-center gap-2.5">
          <div className="h-9 w-9 rounded-lg flex items-center justify-center text-sidebar-primary-foreground font-bold text-sm shrink-0"
               style={{ background: "var(--gradient-primary)" }}>
            A
          </div>
          {!collapsed && (
            <div className="flex flex-col leading-tight">
              <span className="font-semibold text-sidebar-foreground text-sm">Azas</span>
              <span className="text-[10px] text-sidebar-foreground/60 uppercase tracking-wider">Agency CRM</span>
            </div>
          )}
        </div>
      </SidebarHeader>
